...
rmd2html <- function(rmd, modal_dir = here::here("modals")){
  rmd   <- normalizePath(rmd)
  p_htm <- fs::path_ext_set(rmd, ".preglossarized.html")
  p_md  <- fs::path_ext_set(rmd, ".preglossarized.md")
  g_md  <- fs::path_ext_set(rmd, ".glossarized.md")
  htm   <- fs::path_ext_set(rmd, "html")

  # create the default html for fetching header
  render(
    rmd, output_file = p_htm, 
    output_format    = "html_document", 
    output_options   = list(self_contained = F, keep_md = T))
    
  # glossarize
  glossarize_md(p_md, g_md)

  # create the final html file
  render(
    g_md, output_file = htm, 
    output_format    = "html_document", 
    output_options   = list(self_contained = F), clean = T)
  
  # final cleanup
  file.remove(p_htm, p_md, g_md)  
}